...
class CloudScanner:
    ...
    def _setup_credentials(self, account: CloudAccount):
        ...
        if account.provider == CloudProvider.AWS:
            ...
            self.scoutsuite.credential_manager.setup_gcp_credentials(
                service_account_key_path=creds.get("service_account_key_path", ""),
    ...
    def scan_all_accounts(
        ...
        for account in accounts:
            # Async-Scan starten
            asyncio.create_task(
                self.scan_account(account.id, scan_type)
            )
            ...
            while self.monitoring_enabled:
                ...
                for scan_id, scheduled in self.scheduled_scans.items():
                    ...
                    if scheduled.next_run and scheduled.next_run <= now:
                        # Führe Scan durch
                        asyncio.create_task(
                            self.scan_account(scheduled.account_id, scheduled.scan_type)
                        )
    ...
    def generate_compliance_report(
        ...
        try:
            compliance_results = self.scoutsuite.compliance_checker.check_compliance(
                result.report,
                frameworks
            )
