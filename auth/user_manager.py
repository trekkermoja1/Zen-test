...
from database.auth_models import (
    User, UserSession, TokenBlacklist, UserAuditLog, APIKey, MFADevice,
    UserRole, TokenType as DBTokenType, get_auth_db
)
...
class UserManager:
    ...
    def authenticate_user(self, db: Session, username: str, password: str,
        ...
        if not user:
            ...
            self._log_audit(db, None, "login", "auth", ip_address, user_agent,
                           result="failure", failure_reason="user_not_found")
            return None
        
        # Check if account is locked
        if user.locked_until and datetime.utcnow() < user.locked_until:
            self._log_audit(db, user.id, "login", "auth", ip_address, user_agent,
                           result="failure", failure_reason="account_locked")
            return None
        
        # Check if account is active
        if not user.is_active:
            self._log_audit(db, user.id, "login", "auth", ip_address, user_agent,
                           result="failure", failure_reason="account_inactive")
            return None
        ...
        if not self.pwd.verify(password, user.hashed_password):
            ...
            self._log_audit(db, user.id, "login", "auth", ip_address, user_agent,
                           result="failure", failure_reason="invalid_password",
    ...
    def create_user(self, db: Session, username: str, email: str, 
        ...
        # Log creation
        self._log_audit(db, created_by, "user_create", "account", 
                       details={"created_user_id": user.id, "role": role.value})
        
        return user
    
    def change_password(self, db: Session, user_id: int, old_password: str, 
                        new_password: str, ip_address: str = None) -> bool:
        """
        ...
        if not self.pwd.verify(old_password, user.hashed_password):
            self._log_audit(db, user_id, "password_change", "security", ip_address,
                           result="failure", failure_reason="invalid_old_password")
            return False
    ...
    def revoke_all_user_sessions(self, db: Session, user_id: int, 
                                  reason: str = "security") -> int:
        """
    ...
    def get_user_sessions(self, db: Session, user_id: int, 
                          active_only: bool = True) -> List[UserSession]:
    ...
    def blacklist_token(self, db: Session, jti: str, token_type: DBTokenType,
                        expires_at: datetime, user_id: int = None,
                        reason: str = "logout") -> TokenBlacklist:
    ...
    def deactivate_user(self, db: Session, user_id: int, 
        ...
        self._log_audit(db, user_id, "user_deactivate", "account",
                       details={"reason": reason})
        
        return True
    
    def update_user_role(self, db: Session, user_id: int, 
        ...
        self._log_audit(db, user_id, "role_change", "account",
    ...
    def _log_audit(self, db: Session, user_id: int = None, action: str = None,
                   action_category: str = None, ip_address: str = None,
    ...
    def get_user_audit_logs(self, db: Session, user_id: int, 
                            limit: int = 100) -> List[UserAuditLog]:
        """Get audit logs for a user"""
        return db.query(UserAuditLog).filter(
            UserAuditLog.user_id == user_id
        ).order_by(desc(UserAuditLog.timestamp)).limit(limit).all()
...
def get_user_manager(jwt_handler: Optional[JWTHandler] = None,
                     password_hasher: Optional[PasswordHasher] = None) -> UserManager:
