...
def get_repo_stats():
    ...
    if first_commit_date != "N/A":
        try:
            ...
            stats['repo_age_display'] = f"{age_days} days" if age_days < 365 else f"{age_days // 365}y {(age_days % 365) // 30}m"
        except:
...
def generate_status_card():
    ...
    # Footer
    draw.text((width//2, height-25), "🤖 Auto-generated by GitHub Actions | Updates on every push", 
              fill=secondary_text, font=font_small, anchor="mm")
