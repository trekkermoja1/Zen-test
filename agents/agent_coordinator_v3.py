...
class Agent(ABC):
    ...
    async def broadcast(
        self, msg_type: MessageType, payload: Dict[str, Any], priority: int = 1
    ):
...
class AgentCoordinator:
    ...
    def __init__(
        self, max_concurrent_tasks: int = 100, heartbeat_timeout: int = 60
    ):
    ...
    async def register_agent(self, agent: Agent) -> bool:
        ...
        async with self._agent_lock:
            if agent.agent_id in self._agents:
                logger.warning(
                    f"[AgentCoordinator] Agent {agent.agent_id} bereits registriert"
                )
                return False
            ...
            logger.info(
                f"[AgentCoordinator] Agent {agent.name} ({agent.agent_id}) registriert"
            )
    ...
    async def get_agents_by_type(self, agent_type: AgentType) -> List[Agent]:
        ...
        async with self._agent_lock:
            return [
                self._agents[aid]
                for aid in self._agents_by_type.get(agent_type, [])
    ...
    async def route_message(self, msg: AgentMessage):
        ...
        if (datetime.now() - msg.timestamp).total_seconds() > msg.ttl:
            logger.debug(
                f"[AgentCoordinator] Nachricht {msg.msg_id} abgelaufen"
            )
            return
    ...
    async def _send_direct_message(self, msg: AgentMessage):
        ...
        if msg.recipient_id in self._message_router:
            ...
            logger.warning(
                f"[AgentCoordinator] Empfänger {msg.recipient_id} nicht gefunden"
            )
    ...
    async def _handle_coordinator_message(self, msg: AgentMessage):
        ...
        if msg.msg_type == MessageType.RESULT:
            ...
            if task_id:
                ...
                await self._broadcast_event(
                    "task_completed", {"task_id": task_id, "status": "success"}
                )
            ...
            if task_id:
                ...
                await self._broadcast_event(
                    "task_failed",
                    {"task_id": task_id, "error": msg.payload.get("error")},
                )
    ...
    async def assign_task(
        ...
        if preferred_agents:
            available = [
                a for a in available if a.agent_id in preferred_agents
            ]
        ...
        if agent:
            ...
            logger.info(
                f"[AgentCoordinator] Task {task_id} an {best_agent.agent_id} zugewiesen"
            )
            return task_id
    ...
    async def execute_workflow(
        ...
        logger.info(
            f"[AgentCoordinator] Workflow {workflow_id} gestartet (exec={execution_id})"
        )
        return execution_id
                ...
                if step_type == "task":
                    ...
                    if task_id:
                        result = await self.wait_for_task(
                            task_id, timeout=step.get("timeout", 300)
                        )
                        execution["results"][
                    ...
                    results = await asyncio.gather(
                        *[
                            self.wait_for_task(
                                tid, timeout=step.get("timeout", 300)
                            )
                            for tid in tasks
                        ]
                    )
                    ...
                    # Sende Event
                    await self._broadcast_event(
                        step.get("event_name"), step.get("event_data", {})
                    )
    ...
    def _evaluate_condition(self, condition: str, results: Dict) -> bool:
        ...
        try:
            ...
            return eval(
                condition, {"results": results, "__builtins__": {}}
            )  # nosec B307
        except Exception:
            return False
        ...
        async with self._agent_lock:
            ...
            for agent_id, info in list(self._agent_info.items()):
                ...
                if elapsed > self.heartbeat_timeout:
                    logger.warning(
                        f"[AgentCoordinator] Agent {agent_id} timeout"
                    )
            ...
            return {
                **self._stats,
                "agents_online": len(
                    [
                        a
                        for a in self._agent_info.values()
                ...
                "active_workflows": len(
                    [
                        w
                        for w in self._workflow_executions.values()
                        if w["status"] == "running"
                    ]
                ),
            }
