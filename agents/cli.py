        ...
        while True:
            try:
                ...
                if command in ("quit", "exit"):
                    print(
                        colorize("[*] Shutting down agent system...", "yellow")
                    )
                    ...
                    if not args:
                        ...
                        continue
                    print(
                        colorize(f"[*] Starting research on: {args}", "cyan")
                    )
                    ...
                    print(
                        colorize(
                            f"[+] Research thread started: {thread_id}",
                            "green",
                        )
                    )
                    print(
                    ...
                    print(
                        colorize(
                            f"[*] Facilitating agent discussion on: {args}",
                            "cyan",
                        )
                    )
                    ...
                    print(
                        f"  Role distribution: {status.get('role_distribution', {})}"
                    )

                elif command == "agents":
                    status = self.integration.get_system_status()
                    print(colorize("[*] Active Agents:", "cyan"))
                    for agent_id, agent_status in status.get(
                        "agents", {}
                    ).items():
                    ...
                    if not args or len(args.split()) < 2:
                        print(
                            colorize("[!] Usage: context <key> <value>", "red")
                        )
                        continue
                    ...
                    print(
                        colorize(
                            f"[+] Shared context: {key} = {value}", "green"
                        )
                    )
