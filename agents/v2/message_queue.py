...
class AgentMessageQueue:
    ...
    async def publish(
        ...
        if persist:
            ...
            await self.redis.zadd(
                self.KEY_QUEUE.format(channel=channel),
                {queued.id: score}
            )
    ...
    async def receive(
        ...
        processing_key = self.KEY_PROCESSING.format(channel=channel)
...
async def test_in_memory_queue():
    ...
    from .secure_message import MessageHeader, EncryptedPayload
