...
class MessageEncryption:
    ...
    def encrypt(
        ...
        full_ciphertext = ephemeral_public.public_bytes(
            encoding=serialization.Encoding.Raw,
            format=serialization.PublicFormat.Raw
        ) + ciphertext
    ...
    def create_message(
        ...
        data_to_sign = (
            json.dumps(header.to_dict(), sort_keys=True).encode('utf-8') +
            json.dumps(encrypted_payload.to_dict(), sort_keys=True).encode('utf-8')
        )
...
def test_encryption_roundtrip():
    ...
    decrypted = bob_codec.encryption.decrypt(
        encrypted, 
        X25519PrivateKey.from_private_bytes(bob_private)
    )
