...
from database.auth_models import APIKey as APIKeyModel, UserRole, SessionLocal
...
class AgentAuthenticator:
    ...
    def generate_api_key(self, role: AgentRole, name: str, 
                         description: Optional[str] = None,
                         expires_days: Optional[int] = None,
                         rate_limit: int = 1000) -> AgentCredentials:
        """
    ...
    def authenticate(self, api_key: str, api_secret: str) -> Optional[AgentIdentity]:
        ...
        key_model = self.db.query(APIKeyModel).filter(
    ...
    def revoke_key(self, agent_id: str, reason: str = "admin_action") -> bool:
        ...
        key_models = self.db.query(APIKeyModel).filter(
            APIKeyModel.name.like(f"%{agent_id}%")
        ).all()
    ...
    def check_permission(self, identity: AgentIdentity, 
                         permission: AgentPermission) -> bool:
