...
class TimestampMixin(BaseModel):
    ...
    expires_at: Optional[datetime] = Field(
        default=None,
        description="Ablaufzeitpunkt der Nachricht"
    )
...
class AgentIdentity(ACPBaseModel):
    ...
    capabilities: List[str] = Field(
        default_factory=list,
        description="Fähigkeiten des Agenten"
    )
...
class MessageHeader(ACPBaseModel):
    ...
    correlation_id: Optional[str] = Field(
        default=None,
        description="Korrelations-ID für Request-Response-Paare"
    )
...
class MessageSecurity(ACPBaseModel):
    ...
    signature: Optional[str] = Field(
        default=None,
        description="Digitale Signatur der Nachricht"
    )
    ...
    key_id: Optional[str] = Field(
        default=None,
        description="ID des verwendeten Schlüssels"
    )
...
class ACPMessage(ACPBaseModel, TimestampMixin, MetadataMixin):
    ...
    security: MessageSecurity = Field(
        default_factory=MessageSecurity,
        description="Sicherheitsinformationen"
    )
    
    # Tracking-Felder
    delivery_attempts: int = Field(
        default=0,
        description="Anzahl der Zustellungsversuche",
        ge=0
    )
    last_error: Optional[str] = Field(
        default=None,
        description="Letzter Fehler bei der Zustellung"
    )
...
class TaskPayload(ACPBaseModel):
    ...
    status: TaskStatus = Field(
        ...
        description="Aktueller Task-Status"
    )
    assigned_to: Optional[str] = Field(
        default=None,
        description="Zugewiesener Agent"
    )
    parent_task_id: Optional[str] = Field(
        default=None,
        description="ID der übergeordneten Task"
    )
    dependencies: List[str] = Field(
        default_factory=list,
        description="Abhängige Task-IDs"
    )
    instructions: str = Field(
        default="",
        description="Ausführungsanweisungen",
        max_length=50000
    )
    ...
    deadline: Optional[datetime] = Field(
        default=None,
        description="Deadline für die Task"
    )
    progress: int = Field(
        default=0,
    ...
    result: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Task-Ergebnis"
    )
    error_message: Optional[str] = Field(
        default=None,
        description="Fehlermeldung bei Fehlschlag"
    )
...
class StatusPayload(ACPBaseModel):
    ...
    agent_status: AgentStatus = Field(
        ...,
        description="Status des Agenten"
    )
    current_task_id: Optional[str] = Field(
        default=None,
        description="ID der aktuellen Task"
    )
    queue_size: int = Field(
        default=0,
        description="Größe der Message-Queue",
        ge=0
    )
    memory_usage: Optional[float] = Field(
        default=None,
        description="Speichernutzung in MB",
        ge=0
    )
    cpu_usage: Optional[float] = Field(
        default=None,
        description="CPU-Nutzung in Prozent",
        ge=0,
        le=100
    )
    ...
    capabilities_available: List[str] = Field(
        default_factory=list,
        description="Verfügbare Fähigkeiten"
    )
...
class ErrorPayload(ACPBaseModel):
    ...
    stack_trace: Optional[str] = Field(
        default=None,
        description="Stack-Trace (nur bei internen Fehlern)"
    )
    recoverable: bool = Field(
        default=True,
        description="Ob der Fehler behebbar ist"
    )
...
class FileTransferPayload(ACPBaseModel):
    ...
    checksum: str = Field(
        ...,
        description="SHA-256 Prüfsumme der Datei"
    )
    chunk_index: int = Field(
        default=0,
        description="Index des aktuellen Chunks",
        ge=0
    )
    total_chunks: int = Field(
        default=1,
        description="Gesamtanzahl der Chunks",
        ge=1
    )
    chunk_data: Optional[str] = Field(
        default=None,
        description="Base64-kodierte Chunk-Daten"
    )
    compression: Optional[str] = Field(
        default=None,
        description="Kompressionsalgorithmus"
...
class DataRequestPayload(ACPBaseModel):
    ...
    data_type: str = Field(
        ...,
        description="Typ der angeforderten Daten"
    )
    filters: Dict[str, Any] = Field(
        default_factory=dict,
        description="Filterkriterien"
    )
    limit: int = Field(
        default=100,
...
class DataResponsePayload(ACPBaseModel):
    ...
    request_id: str = Field(
        ...,
        description="ID der ursprünglichen Anfrage"
    )
    total_count: int = Field(
        default=0,
        description="Gesamtanzahl der verfügbaren Ergebnisse",
        ge=0
    )
...
class KeyExchangePayload(ACPBaseModel):
    ...
    public_key: str = Field(
        ...,
        description="Öffentlicher Schlüssel (Base64)"
    )
    key_algorithm: str = Field(
        default="ECDH",
        description="Schlüsselaustausch-Algorithmus"
    )
    key_id: str = Field(
        ...,
        description="Eindeutige Schlüssel-ID"
    )
...
class AuthPayload(ACPBaseModel):
    ...
    permissions: List[str] = Field(
        default_factory=list,
        description="Gewährte Berechtigungen"
    )
    expires_at: Optional[datetime] = Field(
        default=None,
