...
class HealthAnalyzer:
    ...
    def _analyze_issues(self) -> None:
        ...
        critical_count = sum(
            ...
            if any(label["name"].lower() in critical_labels for label in i.get("labels", []))
        )
    ...
    def _analyze_prs(self) -> None:
        ...
        print(f"   ✓ PRs: {len(open_prs)} open, {len(stale_prs)} stale, {len(conflict_prs)} conflicts, score: {self.metrics.prs_score:.1f}")
    ...
    def _analyze_branches(self) -> None:
        ...
        print(f"   ✓ Branches: {total_branches} total, {protected_count} protected, {orphaned_count} orphaned, score: {self.metrics.branches_score:.1f}")
    ...
    def _analyze_actions(self) -> None:
        ...
        print(f"   ✓ Actions: {len(recent_runs)} runs, {success_rate:.1f}% success rate, score: {self.metrics.actions_score:.1f}")
    ...
    def _analyze_security(self) -> None:
        ...
        print(f"   ✓ Security: Dependabot={security_details['dependabot_enabled']}, CodeQL={security_details['codeql_enabled']}, Vulns={security_details['vulnerabilities']}, score: {self.metrics.security_score:.1f}")
    ...
    def _analyze_sync(self) -> None:
        ...
        if parent:
            ...
            comparison = self.api.get(f"compare/{parent['owner']['login']}:{parent_default}...{repo_info.get('default_branch', 'main')}")
        ...
        print(f"   ✓ Sync: Fork, behind by {self.metrics.sync_details.get('behind_by', 'unknown')}, score: {self.metrics.sync_score:.1f}")
    ...
    def _generate_recommendations(self) -> None:
        ...
        if self.metrics.issues_details.get("stale", 0) > 5:
            recommendations.append(f"📝 Close {self.metrics.issues_details['stale']} stale issues (inactive > {STALE_ISSUE_DAYS} days)")
        ...
        if self.metrics.actions_details.get("success_rate", 100) < 80:
            recommendations.append(f"⚙️ Investigate workflow failures (current success rate: {self.metrics.actions_details.get('success_rate', 0)}%)")
