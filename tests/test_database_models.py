...
from database.models import User, Scan, Finding, Report, AuditLog
...
class TestUserModel:
    ...
    def test_user_creation(self):
        user = User(username="testuser", email="test@example.com", hashed_password="hashedpass123", role="user")
...
class TestScanModel:
    ...
    def test_scan_creation(self):
        scan = Scan(name="Test Scan", target="example.com", scan_type="web", status="pending", user_id=1)
...
class TestReportModel:
    ...
    def test_report_creation(self):
        report = Report(scan_id=1, format="pdf", status="generating", file_path="/reports/report_1.pdf")
...
class TestAuditLogModel:
    ...
    def test_audit_log_creation(self):
        log = AuditLog(user_id=1, action="login", resource="auth", details={"ip": "192.168.1.1"})
