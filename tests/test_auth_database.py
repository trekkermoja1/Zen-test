...
from database.auth_models import (
    Base, User, UserSession, TokenBlacklist, UserAuditLog,
    UserRole, TokenType, MFADevice, APIKey,
    create_user, get_user_by_username, get_user_by_email,
    create_session, revoke_session, blacklist_token, is_token_blacklisted
)
...
class TestUserSession:
    ...
    def test_create_session(self, db):
        ...
        session = create_session(
            db, user.id, "session_123", "jti_456",
    ...
    def test_session_expiration(self, db):
        ...
        expired_session = create_session(
            db, user.id, "expired_session", "jti_expired",
        ...
        active_session = create_session(
            db, user.id, "active_session", "jti_active",
            datetime.utcnow() + timedelta(days=7),
            ip_address="192.168.1.1"
        )
...
class TestUserManager:
    ...
    def test_create_session_with_tokens(self, db, user_manager):
        ...
        session = db.query(UserSession).filter(
            UserSession.session_id == result["session_id"]
        ).first()
