"""
Comprehensive tests for core/cache.py - Target: 80%+ Coverage

Tests cover:
1. MemoryCache class (get, set, delete, clear, expiration)
2. SQLiteCache class (persistence, queries, cleanup)
3. RedisCache class (mocked async operations)
4. MultiTierCache class (L1/L2/L3 caching, promotion/demotion)
5. Cache statistics and metrics
6. Async cache operations

Run with: pytest tests/unit/core/test_cache_comprehensive.py -v --cov=core.cache
"""
...
def mock_redis():
    ...
    mock_pool.pipeline = MagicMock(
        return_value=AsyncMock(execute=AsyncMock(return_value=[True, True]))
    )
...
class TestCacheStats:
    ...
    def test_hit_rate_calculation(self):
        ...
        stats = CacheStats(hits=80, misses=20, total_gets=100)
...
class TestCacheBackend:
    ...
    async def test_default_mget(self):
        """Test default mget implementation with mocked get."""
        backend = CacheBackend()
        # Mock get to return None (simulating abstract implementation)
    ...
    async def test_default_mset(self):
        """Test default mset implementation with mocked set."""
        backend = CacheBackend()
        # Mock set to return True (simulating abstract implementation)
...
class TestMemoryCacheInitialization:
    ...
    def test_init_custom(self):
        ...
        cache = MemoryCache(max_size=500, max_memory_mb=50.0, default_ttl=3600)
...
class TestMemoryCacheLRU:
    """Tests for LRU eviction in MemoryCache."""
...
class TestSQLiteCacheInitialization:
    """Tests for SQLiteCache initialization."""

    def test_sqlite_init_defaults(self):
    ...
    def test_sqlite_custom_path(self, tmp_path):
        """Test SQLiteCache with custom path."""
        custom_path = tmp_path / "test_cache.db"
...
@pytest.mark.skipif(not SQLITE_AVAILABLE, reason="aiosqlite not installed")
class TestSQLiteCacheBatch:
    """Tests for batch operations in SQLiteCache."""
    ...
    @pytest.mark.asyncio
    async def test_mget_empty(self, sqlite_cache):
        """Test mget with empty list."""
        results = await sqlite_cache.mget([])
        assert results == {}
    ...
    @pytest.mark.asyncio
    async def test_mset_empty(self, sqlite_cache):
        """Test mset with empty dict."""
        result = await sqlite_cache.mset({})
        assert result is True
...
class TestRedisCacheMocked:
    ...
    @pytest.mark.asyncio
    async def test_mget_empty(self, mock_redis):
        """Test mget with empty keys."""
        with patch("core.cache.REDIS_AVAILABLE", True):
            cache = RedisCache()
            cache._pool = mock_redis
    ...
    @pytest.mark.asyncio
    async def test_mset_empty(self, mock_redis):
        """Test mset with empty items."""
        with patch("core.cache.REDIS_AVAILABLE", True):
            cache = RedisCache()
            cache._pool = mock_redis
...
class TestMultiTierCache:
    ...
    @pytest.mark.asyncio
    async def test_tiered_set_persistent(self):
        """Test setting in persistent tier."""
        if not SQLITE_AVAILABLE:
            pytest.skip("aiosqlite not available")

        cache = MultiTierCache(memory_size=10, memory_max_mb=5.0)

        result = await cache.set("key1", "value1", tiers="persistent")
        assert result is True
    ...
    def test_init_without_sqlite(self):
        """Test MultiTierCache without SQLite."""
        with patch("core.cache.SQLITE_AVAILABLE", False):
...
class TestCachedDecorator:
    ...
    @pytest.mark.asyncio
    async def test_sync_function_caching(self):
        """Test caching sync function results with a fresh cache instance."""
        call_count = 0
        
        # Create a fresh cache instance to avoid conflicts with other tests
        fresh_cache = MemoryCache()

        @cached(backend=fresh_cache, ttl=3600)
        ...
        # First call
        result1 = await test_function(7)  # Use different value to avoid conflicts
        assert result1 == 21
        assert call_count == 1

        # Second call - should be cached
        result2 = await test_function(7)
        assert result2 == 21
        assert call_count == 1  # Not called again
...
class TestGetCacheBackend:
    ...
    def test_get_sqlite_backend(self):
        ...
        original = cache_module._sqlite_cache
        cache_module._sqlite_cache = None
        ...
        try:
            ...
            cache_module._sqlite_cache = original
...
class TestCacheEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_large_value_rejection(self):
        """Test that oversized values are rejected."""
        # Create a cache with very small memory limit (0.001 MB = ~1KB)
        cache = MemoryCache(max_size=100, max_memory_mb=0.001)
        
        # Create a value larger than 50% of max memory
