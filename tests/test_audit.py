...
from audit.logger import AuditLogger, AuditLogEntry, LogLevel, EventCategory
from audit.config import AuditConfig, RetentionPolicy
from audit.compliance import ComplianceReporter, ComplianceStandard, ComplianceControl, ComplianceFinding
from audit.siem import (
    SIEMIntegration, SIEMConfig, SplunkBackend, 
    ElasticsearchBackend, QRadarBackend, GenericHTTPBackend
)
...
class TestComplianceReporter:
    ...
    def mock_logger(self):
        ...
        logger.verify_integrity = AsyncMock(return_value={
            "total_entries": 0,
            "valid_signatures": 0,
    ...
    def test_summarize_by_category(self, reporter):
        ...
        logs = [
            AuditLogEntry(id="1", timestamp=datetime.utcnow(), level="info", category="system", event_type="test", message="Test"),
            AuditLogEntry(id="2", timestamp=datetime.utcnow(), level="info", category="system", event_type="test", message="Test"),
            AuditLogEntry(id="3", timestamp=datetime.utcnow(), level="info", category="security", event_type="test", message="Test"),
        ]
