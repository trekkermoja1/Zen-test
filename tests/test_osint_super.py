...
from modules.osint_super import (
    OSINTSuperModule,
    OSINTSuperResult,
)
...
def osint_module(temp_output_dir):
    ...
    with patch("modules.osint_super.SherlockIntegration") as mock_sherlock, \
...
class TestOSINTSuperModule:
    ...
    def test_initialization(self, temp_output_dir):
        ...
        with patch("modules.osint_super.SherlockIntegration"), \
...
async def test_full_investigation_workflow(temp_output_dir):
    ...
    with patch("modules.osint_super.SherlockIntegration") as mock_sherlock_class, \
        ...
        # Setup all mocks
        mock_sherlock = Mock()
        mock_sherlock.search = AsyncMock(return_value=Mock(
            success=True,
            username="testuser",
        ...
        mock_ignorant = Mock()
        mock_ignorant.check_email = AsyncMock(return_value=Mock(
            success=True,
        ...
        mock_subfinder = Mock()
        mock_subfinder.enumerate = AsyncMock(return_value=Mock(
            subdomains=["www.example.com"],
        ))
        mock_subfinder_class.return_value = mock_subfinder
        
        mock_amass = Mock()
        mock_amass.enumerate = AsyncMock(return_value=Mock(
            subdomains=["api.example.com"],
        ))
        mock_amass_class.return_value = mock_amass
        
        mock_whatweb = Mock()
        mock_whatweb.scan = AsyncMock(return_value=Mock(
