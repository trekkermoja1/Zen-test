...
class TestScanWorkflow:
    ...
    async def test_get_scan_status(self, authenticated_client):
        ...
        # Scan erstellen
        create_response = await authenticated_client.post('/api/v1/scans', json={
            'target': 'scanme.nmap.org',
            'scan_type': 'reconnaissance'
        })
...
class TestAgentWorkflow:
    ...
    async def test_register_agent(self, authenticated_client):
        ...
        response = await authenticated_client.post('/api/v1/agents/register', json={
            'name': 'recon_agent_1',
    ...
    async def test_assign_task_to_agent(self, authenticated_client):
        ...
        # Agent registrieren
        agent_response = await authenticated_client.post('/api/v1/agents/register', json={
            'name': 'test_agent',
        ...
        # Task zuweisen
        response = await authenticated_client.post(f'/api/v1/agents/{agent_id}/tasks', json={
            'type': 'vuln_scan',
            'target': 'example.com',
            'priority': 'high',
            'options': {
...
class TestReportWorkflow:
    ...
    async def test_generate_report(self, authenticated_client):
        ...
        scan_response = await authenticated_client.post('/api/v1/scans', json={
            'target': 'scanme.nmap.org',
            'scan_type': 'vulnerability'
        })
        ...
        # Report generieren
        response = await authenticated_client.post('/api/v1/reports', json={
            'scan_id': scan_id,
            'format': 'pdf',
            'template': 'executive',
    ...
    async def test_download_report(self, authenticated_client):
        ...
        # Report erstellen
        report_response = await authenticated_client.post('/api/v1/reports', json={
...
class TestWebhookIntegration:
    ...
    async def test_webhook_delivery(self, authenticated_client):
        ...
        webhook_response = await authenticated_client.post('/api/v1/webhooks', json={
        ...
        with patch('httpx.AsyncClient.post') as mock_post:
            ...
            await authenticated_client.post('/api/v1/scans', json={
                'target': 'scanme.nmap.org',
                'scan_type': 'reconnaissance'
            })
...
class TestSIEMIntegration:
    ...
    async def test_configure_siem(self, authenticated_client):
        ...
        response = await authenticated_client.post('/api/v1/integrations/siem', json={
            'type': 'splunk',
    ...
    async def test_send_finding_to_siem(self, authenticated_client):
        ...
        with patch('httpx.AsyncClient.post') as mock_post:
            ...
            response = await authenticated_client.post('/api/v1/findings/send-to-siem', json={
...
class TestComplianceWorkflow:
    ...
    async def test_compliance_scan(self, authenticated_client):
        ...
        response = await authenticated_client.post('/api/v1/compliance/scans', json={
    ...
    async def test_compliance_report(self, authenticated_client):
        ...
        scan_response = await authenticated_client.post('/api/v1/compliance/scans', json={
            'target': 'internal-network',
        ...
        # Report generieren
        response = await authenticated_client.post('/api/v1/compliance/reports', json={
            'scan_id': scan_id,
            'format': 'pdf',
