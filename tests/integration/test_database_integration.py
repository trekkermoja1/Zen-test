...
class TestTransactions:
    ...
    def test_transaction_commit(self, db):
        ...
        with db.transaction() as tx:
            tx.execute(
                "INSERT INTO scans (scan_id, target) VALUES (?, ?)",
                ('tx-scan-1', 'tx-target.com')
            )
    ...
    def test_nested_transactions(self, db):
        ...
        with db.transaction() as tx1:
            tx1.execute(
                "INSERT INTO scans (scan_id, target) VALUES (?, ?)",
                ('nested-1', 'target1.com')
            )
            
            with db.transaction() as tx2:
                tx2.execute(
                    "INSERT INTO scans (scan_id, target) VALUES (?, ?)",
                    ('nested-2', 'target2.com')
                )
...
class TestBackupRestore:
    ...
    def db(self):
        ...
        for i in range(10):
            db.create_scan({
                'scan_id': f'backup-scan-{i}',
                'target': f'backup{i}.com'
            })
