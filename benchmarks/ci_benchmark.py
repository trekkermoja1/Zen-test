...
class CIBenchmarkRunner:
        ...
        for gate in self.config.performance_gates:
            ...
            result = GateResult(
                ...
                message=(
                    ...
                    if passed
                    else f"❌ {gate.name}: {value:.3f} < {gate.threshold}"
                ),
            )
    ...
    def _get_baseline_report(self, current: BenchmarkReport) -> Optional[BenchmarkReport]:
        ...
        for entry in reversed(history):
            if (
                entry.get("benchmark_id") != current.benchmark_id
                and entry.get("scenarios") == current.config.scenarios
    ...
    def generate_summary_markdown(self, report: BenchmarkReport) -> str:
