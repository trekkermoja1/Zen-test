...
class CompetitorResult:
    ...
    def calculate_metrics(self, expected_findings: List[ExpectedFinding]) -> None:
        ...
        found_set = set(
            f"{f.vuln_type}:{f.location}" for f in self.findings
        )
...
class ComparisonMetrics:
    ...
    def calculate_improvement(self, competitor_name: str, metric: str) -> float:
        ...
        competitor = next(
            (c for c in self.competitor_results if c.tool_name == competitor_name),
            None
        )
...
class TestScenarioValidator:
    ...
    def validate_result(
        ...
        if result.detection_rate < test_case.min_detection_rate:
            errors.append(
                f"Detection rate {result.detection_rate:.2%} below minimum "
                f"{test_case.min_detection_rate:.2%}"
            )
        
        # Check false positive rate
        if result.false_positive_rate > test_case.max_false_positive_rate:
            warnings.append(
                f"False positive rate {result.false_positive_rate:.2%} above maximum "
                f"{test_case.max_false_positive_rate:.2%}"
            )
        
        # Check duration
        if result.duration_seconds > test_case.max_duration_seconds:
            warnings.append(
                f"Duration {result.duration_seconds:.1f}s exceeds maximum "
                f"{test_case.max_duration_seconds}s"
            )
