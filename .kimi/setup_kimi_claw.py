...
def create_or_update_secret(secret_name, secret_value):
    ...
    print(f"\n📋 Secret: {secret_name}")
    print(
        f"   Value: {secret_value[:20]}..."
        if len(str(secret_value)) > 20
        else f"   Value: {secret_value}"
    )
    return True
...
def setup_kimi_claw():
    ...
    print()
    print(
        "Kimi Claw acts as a unified gateway for all repository notifications."
    )
    ...
    secrets_needed = [
        (
            "KIMI_CLAW_TOKEN",
            "API Token from Kimi Claw",
            "Required for Claw integration",
        ),
        (
            "KIMI_CLAW_API_URL",
            "Claw API endpoint",
            "Optional (defaults to Moonshot)",
        ),
...
def test_notification():
    ...
    print(
        f"  gh workflow run kimi-claw-gateway.yml -R {REPO_OWNER}/{REPO_NAME}"
    )
