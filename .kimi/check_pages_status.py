...
def check_pages_status():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Source: {pages_info.get('source', {}).get('branch', 'N/A')}"
            )
        ...
        deploy_response = requests.get(
            deploy_url, headers=headers, params={"environment": "github-pages"}
        )
        ...
        if deploy_response.status_code == 200:
            ...
            if deployments:
                ...
                print(
                    f"Creator: {latest.get('creator', {}).get('login', 'N/A')}"
                )
                ...
                if status_url:
                    ...
                    if status_resp.status_code == 200:
                        ...
                        if statuses:
                            ...
                            print(
                                f"State: {latest_status.get('state', 'Unknown').upper()}"
                            )
                            print(
                                f"Description: {latest_status.get('description', 'N/A')}"
                            )
                            print(
                                f"Updated: {latest_status.get('updated_at', 'N/A')}"
                            )
                        else:
        ...
        runs_resp = requests.get(
            runs_url, headers=headers, params={"per_page": 10}
        )

        if runs_resp.status_code == 200:
            runs = runs_resp.json().get("workflow_runs", [])
            pages_runs = [
                r for r in runs if "pages" in r.get("name", "").lower()
            ]
            ...
            if pages_runs:
                ...
                for run in pages_runs[:3]:
                    ...
                    print(
                        f"  {status_icon} {run.get('name')} - {run.get('conclusion') or run.get('status')}"
                    )
