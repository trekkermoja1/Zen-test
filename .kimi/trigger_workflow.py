...
def trigger_workflow(workflow_id, branch="main"):
    ...
    if response.status_code == 204:
        print(
            f"[OK] Workflow '{workflow_id}' triggered successfully on branch '{branch}'"
        )
        print(
            f"     Check status at: https://github.com/{REPO_OWNER}/{REPO_NAME}/actions"
        )
        return True
    else:
...
def list_workflows():
    ...
    if response.status_code == 200:
        ...
        for wf in workflows:
            ...
            print(
                f"   File: {wf.get('path', '').replace('.github/workflows/', '')}"
            )
            print()
    else:
