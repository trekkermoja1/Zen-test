    ...
    if response.status_code == 200:
        ...
        for alert in alerts:
            ...
            info = {
                ...
                "package": pkg,
                "severity": severity,
                "title": alert.get("security_advisory", {}).get(
                    "summary", "No title"
                ),
                "manifest": alert.get("dependency", {}).get(
        ...
        if high:
            ...
            for h in high:
                print(
                    f"  #{h['number']}: {h['package']} - {h['title'][:50]}..."
                )
        ...
        if medium:
            ...
            for m in medium:
                print(
                    f"  #{m['number']}: {m['package']} - {m['title'][:50]}..."
                )
        ...
        if low:
            ...
            for l in low:
                print(
                    f"  #{l['number']}: {l['package']} - {l['title'][:50]}..."
                )
    ...
    if response.status_code == 200:
        ...
        for alert in alerts:
            ...
            dismiss_resp = requests.patch(
                dismiss_url, headers=headers, json=data
            )
            if dismiss_resp.status_code == 200:
                print(f"  [OK] Dismissed alert #{alert_num}")
            else:
                print(
                    f"  [FAIL] Could not dismiss #{alert_num}: {dismiss_resp.status_code}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dismiss-low",
        action="store_true",
        help="Dismiss low severity alerts",
    )
