...
from fastapi import (
    ...
    Request, status, WebSocket, WebSocketDisconnect
)
...
async def broadcast_scan_update(scan_id: int, update_type: str, data: Dict):
    ...
    await scan_ws_manager.broadcast_to_scan(scan_id, {
        "type": update_type,
        "scan_id": scan_id,
        "timestamp": datetime.utcnow().isoformat(),
        **data
...
async def create_new_scan(
    ...
    try:
        ...
        add_scan_log(db_scan.id, "INFO", "initializing", f"Scan '{request.name}' created", {
            "target": request.target,
            "scan_type": request.scan_type,
            "user": user.get("sub")
        })
        ...
        await broadcast_scan_update(db_scan.id, "created", {
            "name": request.name,
            "target": request.target,
            "status": "pending"
        })
    ...
    try:
        ...
        for phase, description, duration in phases:
            ...
            for i in range(duration):
                ...
                await broadcast_scan_update(scan_id, "progress", {
                    "phase": phase,
