...
async def run_nuclei_scan(
    ...
    cmd_parts = [
        ...
        "-c", str(request.options.get("concurrency", 50)),
        "-rl", str(request.options.get("rate_limit", 150))
    ]
...
async def run_gobuster_scan(
    ...
    cmd_parts = [
        ...
        "-w", wordlist,
        "-o", str(output_file),
        "-t", str(request.options.get("threads", 50))
    ]
...
def parse_nmap_xml(file_path: Path) -> Dict[str, Any]:
    ...
    return {"hosts": hosts, "scan_info": root.find("scaninfo").attrib if root.find("scaninfo") else {}}
