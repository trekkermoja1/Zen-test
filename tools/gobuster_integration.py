...
class GobusterScanner:
    ...
    def dir_scan(self, url: str, wordlist: str = "/usr/share/wordlists/dirb/common.txt",
                 extensions: str = "php,html,js,txt",
                 threads: int = 50) -> List[GobusterResult]:
    ...
    def dns_scan(self, domain: str, wordlist: str = "/usr/share/wordlists/dnsrecon/subdomains-top1mil.txt") -> List[str]:
        """DNS Subdomain Enumeration"""
        cmd = [
            self.gobuster_path, "dns",
            "-d", domain,
            "-w", wordlist,
            "-q"
        ]
...
def gobuster_dns_scan(domain: str) -> str:
    ...
    return f"Found {len(results)} subdomains" if results else "No subdomains found"
